	...
	public ApiResponse addToCart(Cart cart) {
		...
		*/
		...
		for(int i = 0; i < carts.size(); i++){
			Cart cartToCompare = carts.get(i);
			if(cartToCompare.getGtin().equals(cart.getGtin())) {
				int newQuantity = cartToCompare.getQuantity() + cart.getQuantity();
				if(newQuantity > product_stock) {
					throw new ApiException(HttpStatus.BAD_REQUEST, "invalid quantity");
				}else{
					cartToCompare.setQuantity(newQuantity);
					repo.save(cartToCompare);
